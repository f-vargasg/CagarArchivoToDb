    ...
    public class AsignaMercDL : DataWorker
    {
        public void Insertar(AsignaMercBE pParams)
        {
            try
            {
                ...
                {
                    using (DbCommand command = database.CreateStoredProcCommand("VO_PAMBASIGNAMERC.Inserta", connection))
                    {
                        #region Parameters
                        ...
                        command.Parameters.Add(param);
                        string partType = ((pParams.Parttype.CompareTo("Contado") == 0) ? "1": "2");
                        param = database.CreateParameter("PPARTTYPE", DbType.String, partType );
                        command.Parameters.Add(param);
                        param = database.CreateParameter("PCENTRALSECURITYREPOSITORY", DbType.String,
                        pParams.Centralsecurityrepository);
                        command.Parameters.Add(param);
                        param = database.CreateParameter("PCURRENCYCODE", DbType.String, (pParams.Currencycode.CompareTo("Colones") == 0 ? "01" : "02"));
                        command.Parameters.Add(param);
